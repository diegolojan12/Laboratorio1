...
CREATE TABLE Categorias
    ...
    TABLESPACE TS_EDILOJA_CATALOGOS;

ALTER TABLE Categorias
    ADD CONSTRAINT Categoria_PK PRIMARY KEY (cate_id)
    USING INDEX TABLESPACE TS_EDILOJA_INDICES;
...
CREATE TABLE Instalaciones
    ...
    TABLESPACE TS_EDILOJA_HISTORICOS;
...
CREATE INDEX idx_per_estado
    ON Perifericos(per_estado)
    TABLESPACE TS_EDILOJA_INDICES;

CREATE INDEX idx_per_ubicacion
    ON Perifericos(Ubicaciones_ubi_id)
    TABLESPACE TS_EDILOJA_INDICES;

CREATE INDEX idx_per_categoria
    ON Perifericos(Categoria_cate_id)
    TABLESPACE TS_EDILOJA_INDICES;

CREATE INDEX idx_per_proveedor
    ON Perifericos(Provedores_pro_id)
    TABLESPACE TS_EDILOJA_INDICES;

CREATE INDEX idx_asig_periferico
    ON Asignaciones(Perifericos_per_id)
    TABLESPACE TS_EDILOJA_INDICES;

CREATE INDEX idx_asig_encargado
    ON Asignaciones(Encargados_encrd_id)
    TABLESPACE TS_EDILOJA_INDICES;

CREATE INDEX idx_mts_periferico
    ON Mantenimientos(Perifericos_per_id)
    TABLESPACE TS_EDILOJA_INDICES;

CREATE INDEX idx_mts_fechaenvio
    ON Mantenimientos(fechaEnvio)
    TABLESPACE TS_EDILOJA_INDICES;

CREATE INDEX idx_enc_departamento
    ON Encargados(Departamentos_dep_id)
    TABLESPACE TS_EDILOJA_INDICES;

CREATE INDEX idx_inst_periferico
    ON Instalaciones(Perifericos_per_id)
    TABLESPACE TS_EDILOJA_INDICES;



ALTER TABLE Asignaciones
    ADD CONSTRAINT Asignacion_Encargados_FK FOREIGN KEY (Encargados_encrd_id)
    REFERENCES Encargados (encrd_id);

ALTER TABLE Asignaciones
    ADD CONSTRAINT Asignacion_Perifericos_FK FOREIGN KEY (Perifericos_per_id)
    REFERENCES Perifericos (per_id);

ALTER TABLE Encargados
    ADD CONSTRAINT Encargados_Departamentos_FK FOREIGN KEY (Departamentos_dep_id)
    REFERENCES Departamentos (dep_id);

ALTER TABLE Instalaciones
    ADD CONSTRAINT Instalaciones_Perifericos_FK FOREIGN KEY (Perifericos_per_id)
    REFERENCES Perifericos (per_id);

ALTER TABLE Instalaciones
    ADD CONSTRAINT Instalaciones_Softwares_FK FOREIGN KEY (Softwares_soft_id)
    REFERENCES Softwares (soft_id);

ALTER TABLE Mantenimientos
    ADD CONSTRAINT Mantenimientos_Perifericos_FK FOREIGN KEY (Perifericos_per_id)
    REFERENCES Perifericos (per_id);

ALTER TABLE Perifericos
    ADD CONSTRAINT Perifericos_Categoria_FK FOREIGN KEY (Categoria_cate_id)
    REFERENCES Categorias (cate_id);

ALTER TABLE Perifericos
    ADD CONSTRAINT Perifericos_Provedores_FK FOREIGN KEY (Provedores_pro_id)
    REFERENCES Proveedores (pro_id);

ALTER TABLE Perifericos
    ADD CONSTRAINT Perifericos_Ubicaciones_FK FOREIGN KEY (Ubicaciones_ubi_id)
    REFERENCES Ubicaciones (ubi_id);
